    ...
    <section id="top" className="relative flex min-h-screen items-center justify-center overflow-hidden noise">
      ...
      <div className="relative z-10 mx-auto max-w-6xl px-6 text-center">
        ...
        <motion.p initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.7, delay: 0.5 }}
          ...
          {t.roles.map((role, i) => (
            <span key={role}>
              ...
              {i < t.roles.length - 1 && <span className="mx-3 text-neon">·</span>}
            </span>
          ))}
        </motion.p>
        ...
        <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.7, delay: 0.7 }}
          ...
          <a href="/cv/Radhif-Aryant-Aurofiq-CV.pdf" download target="_blank" rel="noopener noreferrer"
        ...
        <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} transition={{ delay: 1.4, duration: 1 }}
          ...
          <div className="flex flex-col items-center gap-2 text-xs text-muted-foreground">
            <span className="font-mono tracking-widest">{t.scroll}</span>
